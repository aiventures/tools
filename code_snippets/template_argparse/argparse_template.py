""" argparse templater WORK IN PROGRESS / might be replaced """
...
class Runner():
    ...
    def __init__(self,f_config:str=None,f_params:str=None) -> None:
        """ Constructor """
        self._path = Path(__file__)
        self._cwd = os.getcwd()
        self._config={}
        self._params={}
        # self._read_params(f_config)
    ...
    def _read_config(self,f_config:str=None)->None:
        """ reads config """
        if f_config is None:
            f_config = Runner.DEFAULT_PARAMS
        f_config = self._get_file(f_config)
        if f_config is None:
            return
