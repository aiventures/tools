...
def read_image_meta(f_img:str,attributes="DateTimeOriginal"):
    """
      f: filename of image
    ...
    exiftool_cmd=EXIFTOOL_CMD.replace("EXIFTOOL","exiftool.exe")
...
def get_replace_dict(f_img:str,f_gpx:str,time_gps:str=None,tz:str="Europe/Berlin"):
    ...
    exif_json=read_image_meta(f_img,attributes=DATE_TIME_ORIGINAL)
    ...
    if wpt_min:
        ...
        dt_now_s=DateTime.now().strftime("%Y:%m:%d %H:%M:%S")
...
def create_wpt_file(f_img:str,f_gpx:str,f_waypt_template:str,
    ...
    for k,v in files.items():
        print(f"{k:<15}: {v}")
    replace_dict=get_replace_dict(f_img,f_gpx,time_gps,tz)
