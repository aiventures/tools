...
def read_exif(f:str,exif_fields:str=EXIF_FIELDS,software:str=SOFTWARE,include_entropy=False,debug=False,include_app=False):
    ...
    exifdata=im.getexif()            
    im.close()
    exif_dict=dict([(TAGS.get(tag_id,str(tag_id)),{"tag_id":tag_id,"value":exifdata.get(tag_id,None)}) for tag_id in list(exifdata.keys())])
    if debug:
...
def get_subpath_info_dict(fp:str,type_raw=TYPE_RAW,type_meta=TYPE_META,
    ...
    for subpath,subdirs,files in os.walk(fp):
        ...
        for f in files:
            ...
            if suffix.lower() in type_processed:
